...
# ============================
# Encoding Process (Hide Data)
# ============================
def encode_message():
    image_path = entry_image_path_encode.get()
    password = password_var_encode.get()
...
def decode_message():
    ...
    try:
        ...
        # Enable the text widget, insert the decrypted message, then disable it
        text_message_decode.config(state="normal")
        text_message_decode.delete("1.0", tk.END)
...
# Set the combobox with a grey background, border, and ensure selected text is black.
...
ttk.Button(buttons_frame, text="Encode", command=encode_message, width=10).pack(side="left", padx=5)
...
ttk.Button(decode_group, text="Decode", command=decode_message, width=10)\
...
# Make the decoded message text area read-only (copyable but not editable)
text_message_decode.config(state="disabled")
...
ttk.Button(decode_buttons_frame, text="Decode", command=decode_message, width=10).pack(side="left", padx=5)
...
# ============================
# Set default screen to "Hide Data" (Encode Screen)
# ============================
